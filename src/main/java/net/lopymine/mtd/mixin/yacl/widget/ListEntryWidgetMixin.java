...
public class ListEntryWidgetMixin {
	...
	@WrapWithCondition(at = @At(value = "INVOKE", target = "Ldev/isxander/yacl3/gui/TooltipButtonWidget;extractRenderState(Lnet/minecraft/client/gui/GuiGraphicsExtractor;IIF)V"), method = "extractRenderState")
	private boolean disableRendering1(TooltipButtonWidget instance, GuiGraphicsExtractor context, int x, int y, float v) {
		return !((instance == this.moveDownButton || instance == this.moveUpButton) && (this.listOptionEntry.pendingValue() instanceof SimpleEntry<?, ?>));
	}

//	@WrapWithCondition(at = @At(value = "INVOKE", target = "Ldev/isxander/yacl3/gui/AbstractWidget;extractRenderState(Lnet/minecraft/client/gui/GuiGraphicsExtractor;IIF)V"), method = "extractRenderState")
//	private boolean disableRendering2(TooltipButtonWidget instance, GuiGraphicsExtractor context, int x, int y, float v) {
//		return !((instance == this.moveDownButton || instance == this.moveUpButton) && (this.listOptionEntry.pendingValue() instanceof SimpleEntry<?, ?>));
//	}
